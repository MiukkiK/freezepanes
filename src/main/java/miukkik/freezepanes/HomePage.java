...
public class HomePage extends WebPage {
	...
	private static int FREEZE_COLUMNS = 1;
    ...
    public HomePage(final PageParameters parameters) {
    	...
    	RepeatingView headerContainer = new RepeatingView("headerrow");
		add(headerContainer);
    	for (int i=0; i<FREEZE_ROWS; i++) {
    		...
    		RepeatingView cell = new RepeatingView("headercell");
    		container.add(cell);
    	...
    	for (int i=FREEZE_ROWS; i<table.getTable().size(); i++) {
    		...
    		RepeatingView leftHeader = new RepeatingView("headercolumn");
    		container.add(leftHeader);
    		for (int j=0; j<FREEZE_COLUMNS; j++) {
    			leftHeader.add(new Label(String.valueOf(j), table.getTable().get(i).get(j).getContent()));
    		}
    		RepeatingView cell = new RepeatingView("cell");
    		container.add(cell);
    		for (int j=FREEZE_COLUMNS; j<table.getTable().get(i).size(); j++) {
    			cell.add(new Label(String.valueOf(j), table.getTable().get(i).get(j).getContent()));
    		}
